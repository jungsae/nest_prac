...
import type { CurrentUserType } from 'src/auth/decorators/current-user.decorator';
...
export class UsersController {
    ...
    async findEverything(): Promise<User[]> {
        return await this.usersService.findEverything();
    }
    ...
    async update(@CurrentUser() user: CurrentUserType, @Body() updateUserDto: UpdateUserDto): Promise<boolean> {
        ...
        return await this.usersService.update(userInfo.id, updateUserDto);
    }
    ...
    async remove(@CurrentUser() user: CurrentUserType): Promise<boolean> {
        return await this.usersService.remove(user.email);
    }
}
